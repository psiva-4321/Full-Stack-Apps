...
const Cart = () => {
  ...
  let inccartqty=(cid)=>{
    axios.get(`http://localhost:5000/inccartqty/${cid}`).then((res)=>{
      setF(!f)
    })
  }
  let deletecartitem=(cid)=>{
    axios.delete(`http://localhost:5000/deletecartitem/${cid}`).then((res)=>{
      setF(!f)
    })
  }
  let deccartqty=(cid,qty)=>{
    if(qty>1){
    axios.get(`http://localhost:5000/deccartqty/${cid}`).then((res)=>{  
      setF(!f)
    })
    }
    else{
